...
export const setBooksWithQuizz = (books)=>{
  return {
    type: actionTypes.SET_BOOKS_WITH_QUIZZ,
    books: books,
  }
}
...
export const FetchBooks = () => {
  return async (dispatch) => {
    ...
    dispatch(
      setBooks(Object.values(bookslisting.docs.map((doc) => doc.data())))
    );
    dispatch(
      FilteredBooks(Object.values(bookslisting.docs.map((doc) => doc.data())))
    );
...
export const FetchBooksWithQuiz = (quizesIDS) => {
  ...
  return async (dispatch) => {
    ...
    const bookslisting = await db.collection("bookslist").where("id", "in", quizesIDS).get()
    console.log(bookslisting)
    dispatch(
      setBooksWithQuizz(Object.values(bookslisting.docs.map((doc) => doc.data())))
    );
    // dispatch(
    //   FilteredBooks(Object.values(bookslisting.docs.map((doc) => doc.data())))
    // );
...
export const FetchFirstLast = (currentUser) => {
  return async (dispatch) => {
    ...
    const firstlast = await db
      ...
      .get();
    dispatch(
      FirstLast(firstlast.data()["FirstName"], firstlast.data()["LastName"])
    );
  };
};
